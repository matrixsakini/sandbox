...
- **`audio/`** — the generated clips (~60: one per interactive word × 4 languages, ~5 KB each) plus `manifest.json`. **Do not hand-edit.** Regenerate with `tools/generate_audio.py`.
- **`tools/generate_audio.py`** — offline generator for `audio/`. Reads the vocabulary from `vocab.js` (single source of truth) and renders each word with **eSpeak NG** + **MBROLA** diphone voices (native eSpeak voice for Japanese and as a per-word fallback where MBROLA lacks a diphone). Not part of the runtime or any build step; run by hand when the vocabulary changes. The clips can be upgraded to a premium/neural TTS by swapping the `VOICES`/`synth()` in this script — keep the same output paths and the game picks them up unchanged.
