/*
 * Moon Slash leaderboards — per-mode high-score tables in localStorage.
 * No backend: each browser keeps its own boards. Mirrors the trivia service's
 * semantics — best score per player, sorted descending, capped at the top N.
 */
window.Leaderboard = (() => {
  'use strict';

  const STORAGE_KEY = 'moonslash-leaderboard';
  const NAME_KEY = 'moonslash-lastname';
  const LIMIT = 10;
  const MAX_NAME = 24;
  const MODES = ['prism', 'eternal'];
  ...
  function boardFor(mode) {
    return MODES.includes(mode) ? mode : 'prism';
  }
